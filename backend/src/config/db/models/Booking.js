...
module.exports = new (class BookingModel extends Model {
  ...
  createBooking(data) {
    return new Promise(function (myResolve, myReject) {
      db.query("INSERT INTO booking SET ?", data, function (error, result) {
        if (error) {
          myReject(error);
        } else {
    ...
    return new Promise((resolve, reject) => {
      const query = `
        SELECT
          ...
          d.idRoom
        FROM booking b 
          JOIN bookingDetails d ON  d.idBooking = b.id
          JOIN room r ON  r.id =d.idRoom
          JOIN account a ON a.id = b.idAccount
